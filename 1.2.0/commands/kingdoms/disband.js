module.exports = {
  ...
  run (message, args) {
    const { client: { db: { users, kingdoms } }, user: { id: user_id } } = message
    users.get(`${user_id}.kingdom`).then(async kingdom => {
      ...
      if (role !== 3 || owner !== message.author.id) {
        return message.say(':police_officer: Hold up! You aren\'t the owner of this Kingdom. Only **' + this.client.users.cache.find(user => user.id === owner).tag + `** can disband this Kingdom. If you wish however, you can leave this kingdom by typing \`${this.client.commandPrefix} leave\`.`)
      }
